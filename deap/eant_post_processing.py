...
from eant import Genome
from eant_visualization import export_eant_tree
...
def analyze_single_classifier(exp_path, file_name, exp_name):
    ...
    if len(best_inds) > 0:
        ...
        export_eant_tree(best_inds[-1], rename_labels, graph_file)
        ...
        for i in range(tot):
            ...
            if i < 20:
                export_eant_tree(best_inds[i], rename_labels, graph_file)
            elif i < 100:
                if i % 10 == 0:
                    export_eant_tree(best_inds[i], rename_labels, graph_file)
            else:
...
def make_experiment_dict(exp_path, eant=False, guided=False):
    ...
    type_string = type_string + "_guided" if guided else type_string
    ...
    aggr_dict['Type'] = [type_string for i in range(len(aggr_dict['Avg']))] 
...
def compare_experiments(title, experiments):
    ...
    for (path, eant, guided) in experiments:
        ...
        aggr_dict['Type'] = aggr_dict['Type'] + path_dict['Type']
        aggr_dict['RunIdx'] = aggr_dict['RunIdx'] + path_dict['RunIdx']
        
    # Create the plot using seaborn
    ax = sns.lineplot(data=aggr_dict, x='Generation', y='Med', hue='Type', err_style="band")
    # Set labels and title for the plot
    plt.xlabel('Generation')
    plt.ylabel('Med Fitness')
    plt.title(title)
    ...
    # Create the plot using seaborn
    ax = sns.lineplot(data=aggr_dict, x='Generation', y='Min', hue='Type', err_style="band")
    # Set labels and title for the plot
    plt.xlabel('Generation')
    plt.ylabel('Min Fitness')
    plt.title(title)
    ...
    # Create the plot using seaborn
    ax = sns.lineplot(data=aggr_dict, x='Generation', y='Max', hue='Type', err_style="band")
    # Set labels and title for the plot
    plt.xlabel('Generation')
    plt.ylabel('Max Fitness')
    plt.title(title)
    ...
    # Create the plot using seaborn
    ax = sns.lineplot(data=aggr_dict, x='Generation', y='Avg', hue='Type', err_style="band")
    # Set labels and title for the plot
    plt.xlabel('Generation')
...
if __name__ == '__main__':
    ...
    model_path = str(Path.cwd()) + "/../../../experiments/20230707_eant_vs_gepnn/eant/20230710_iris/"
    #analyze_single_classifier(model_path, "stats_iter_47.pickle", "Testing plotting")
    
    #analyze_all(model_path)
    
    
    # Each experiment is a (path_to_data, eant_experiment?, guided_experiment?)
    experiments = [(str(Path.cwd()) + "/../../../experiments/20230707_eant_vs_gepnn/eant/20230710_iris/", True, False),
                   (str(Path.cwd()) + "/../../../experiments/20230707_eant_vs_gepnn/eant/20230710_iris_guided/", True, True)]
    
    compare_experiments("20230710 Iris EANT", experiments)